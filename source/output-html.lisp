...
(defun slurp-lines (pathname)
  (with-open-file (f pathname :direction :input)
    (loop for line = (read-line f nil f)
          while (not (eq line f))
          collect line)))

(defun write-stylesheet (stream css-file)
  (let ((lines (slurp-lines css-file)))
    (format stream "<style>~%")
    (dolist (line lines)
      (write-line line stream))
    (format stream "</style>~%")))

(defun write-html-toc (document stream &optional (depth most-positive-fixnum))
...
(defun write-html-sidebar (document stream)
  ...
  (write-string "<h2>Table of Contents</h2>" stream)
  (write-string "<nav>" stream)
...
(defmethod write-html ((clause document) stream)
  ...
  (write-string "</header>" stream)
  (fresh-line stream)
  (write-string "<div id=\"main-wrapper\">" stream)
  (write-string "  <div id=\"toc\">" stream)
  (write-html-sidebar clause stream)
  (write-string "  </div>" stream)
  (write-string "  <div id=\"contents-wrapper\">" stream)
  (write-string "    <div id=\"contents\">" stream)
  (write-html (clause-body clause) stream)
  (write-string "    </div>" stream)    ;contents
  (write-string "  </div>" stream)      ;contents-wrapper
  (write-string "</div>" stream)        ;main-wrapper
  (format stream "</body>~%"))

(defmethod write-html ((clause index-section) stream)
  (write-string "<div class=index>" stream)
  (write-string "<h2>" stream)
  (write-html (clause-title clause) stream)
  (write-string "</h2>" stream)
...
(defmethod write-html ((clause section) stream)
  (if (typep (ancestor-of-type clause 'named-clause) 'definition)
    ...
    (let ((tag (case (section-level clause)
      ...
      (write-html (clause-title clause) stream)
      (format stream "</~a>~%" tag)
      (if (<= (section-level clause) 1)
        (format stream "<div class=\"chapter\">~%")
